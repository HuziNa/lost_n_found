...
export default function PizzaCustomizePage() {
  const { productId } = useParams();
  const navigate = useNavigate();
  const { addToCart } = useApp();
  const { user, openAuthModal } = useAuth();
  const canOrder = user?.role === "customer";
  const heroSubtitle = "Craft the crust, sauce, and spice level to match your perfect slice.";
  const orderTitle = "Your Signature Pizza";
          ...
          <div className="order-sidebar">
            <div className="order-card shadow-premium">
              ...
              <div className="order-preview">
                 <CustomizerSVG type={template?.label || "PIZZA"} selections={svgSelections} product={product} />
              </div>
