...
export default function CupcakeCustomizePage() {
  const { productId } = useParams();
  const navigate = useNavigate();
  const { addToCart } = useApp();
  const { user, openAuthModal } = useAuth();
  const canOrder = user?.role === "customer";
  const heroSubtitle = "Pick the base, swirl the frosting, and top it with flair.";
  const orderTitle = "Your Signature Cupcake";
          ...
          <div className="order-sidebar">
            <div className="order-card shadow-premium">
              ...
              <div className="order-preview">
                 <CustomizerSVG type={template?.label || "CUPCAKE"} selections={svgSelections} product={product} />
              </div>
